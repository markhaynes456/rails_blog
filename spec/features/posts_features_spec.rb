...
describe("Post features") do
	...
	it("Linked posts should link to a different website") do
		...
		@posts.each do |post|
			if post.linked != true && passed == true
				unless find_link("View '#{post.title}'")[:href].should == "/posts/#{post.id}"
					passed = false
				end
			else
				unless find_link("View '#{post.title}'")[:href].should == post.link
					passed = false
				end
			end
		end
	...
	it("create a post") do
		...
		expect(page).to(have_content("We made google"))
	end	
